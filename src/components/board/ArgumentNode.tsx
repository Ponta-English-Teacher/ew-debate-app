...
export default function ArgumentNode({
  ...
  const isOwner = argument.student_id === studentId;
  ...
  async function handleCopy() {
    try {
      await navigator.clipboard.writeText(argument.content);
      setCopied(true);
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/arguments/${argument.id}?student_id=${studentId}`, {
        method: 'DELETE',
      });
      if (res.ok) {
        onDeleted?.(argument.id);
      } else {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-3 pb-2 pt-1.5 border-t border-slate-100">
        ...
        <button
          onClick={handleCopy}
          className="flex items-center gap-1 text-[11px] text-slate-400 hover:text-slate-600 transition-colors whitespace-nowrap"
        >
          <CopyIcon />
          {copied ? 'Copied' : 'Copy'}
        </button>
